...
class StudentsController: UIViewController, TANavigationBarType {
    ...
    func setNavigationBar() {
        self.title = "徒弟"
        self.navigationItem.leftBarButtonItem = UIBarButtonItem(image: UIImage(named: "icon_register_second_back"), style: .Plain, target: self, action: #selector(StudentsController.back))
    }
...
extension StudentsController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if self.studentInfos[indexPath.row].avatarURL.hasPrefix("/var/mobile/") || self.studentInfos[indexPath.row].avatarURL.hasPrefix("/Users/") {
