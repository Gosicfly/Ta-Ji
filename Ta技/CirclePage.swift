...
class CirclePage: UICollectionViewCell {
    ...
    lazy var searchController: UISearchController = {
        ...
        searchController.searchBar.placeholder = "圈子名称/圈子号"
...
extension CirclePage: UITableViewDataSource, UITableViewDelegate {
...
extension CirclePage: UISearchBarDelegate {
    
    func searchBarSearchButtonClicked(searchBar: UISearchBar) {
        self.searchBar.resignFirstResponder()
        SVProgressHUD.show()
        Alamofire.request(.GET, "http://taji.whutech.com/User/search?userid=\(TAUtilsManager.userInfoManager.readID().0)&openid=\(TAUtilsManager.userInfoManager.readID().1)&wd=\(self.searchBar.text!)").responseJSON { (response) in
...
extension CirclePage: UISearchResultsUpdating {
